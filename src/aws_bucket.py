...
class S3ProcessingPipeline:
    def __init__(self, bucket_name: str, download_dir: str = "downloads", max_concurrent: int = 2):
    ...
    def verify_bucket_access(self) -> bool:
        ...
        try:
            ...
            if error_code == '403':
                self.error_logger.error(f"No permission to access bucket '{self.bucket_name}'")
            elif error_code == '404':
    ...
    def download_file(self, s3_key: str) -> Optional[Path]:
        ...
        try:
            ...
            local_path = self.download_dir / s3_key  # This preserves the full path structure
    ...
    def start_downloading(self, local_dir: str):
        """Start downloading files from S3"""
        try:
            ...
            for page in paginator.paginate(Bucket=self.bucket_name):
                for obj in page.get('Contents', []):
                    if obj['Key'].lower().endswith('.webm'):
                        self.logger.info(f"Found webm file: {obj['Key']}")
                        local_path = self.download_file(obj['Key'])
                        if local_path:
                            self.logger.info(f"Successfully downloaded: {local_path}")
                        sleep(1)  # Brief pause between downloads
                        
        except Exception as e:
            self.error_logger.error(f"Download error: {str(e)}")
    ...
    #             while not self.processing_queue.empty() and not self.stop_event.is_set():
    #                 futures = []
                    
    #                 # Submit up to max_concurrent files
    #                 while len(futures) < self.max_concurrent and not self.processing_queue.empty():
    ...
    #         self.stop_event.set()
     ...
     # transcriber = AudioTranscriber(config)
        ...
        # Start processing
        # process_files(downloader, transcriber, local_dir, logger)
